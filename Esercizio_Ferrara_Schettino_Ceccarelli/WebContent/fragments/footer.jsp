<link rel="stylesheet" type="text/css" href="./css/Struttura.css">


<footer>
<div class="row">
  <div class="col">
 <h3>Sviluppatori</h3>
   <p><img src="./image/cat.png" class="image" >
 	 Nome: Sabrina Ceccarelli <br>
     Email: s.ceccarelli1@studenti.unisa.it</p><br>
  
   <p><img src="./image/coniglio.png" class="image">
     Nome: Miriam Ferrara <br>
  	 Email: M.Ferrara115@studenti.unisa.it</p><br>
 
  <p><img src="./image/pappagallo.png" class="image">
 	 Nome: Alfonso Schettino <br>
 	 Email: a.schettino23@studenti.unisa.it </p> 
 </div>
 
 <div class="col">
  <h3>Dove ci trovi</h3>
<p>
  Università degli Studi di Salerno<br>
  Via Giovanni Paolo II, 132<br>
  84084 Fisciano (SA)<br>
  Sito web: <a style="color:black" href="https://www.unisa.it">www.unisa.it</a></p> 
  </div>

<div class="col"> 
  <h3>Per conoscerci meglio</h3>
  
  <p><a style="color:black" href="ChiSiamo.jsp">Chi Siamo</a><br>
  Github: <a style="color:black" href="https://github.com/Alfonso-hub/QuaLaZampa.git">QuaLaZampa</a></p>
  </div>
  </div>
  
  <p> &#169; QuaLaZampa 2021</p>
</footer>
